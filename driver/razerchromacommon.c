 ...
 */
struct razer_report razer_chroma_misc_set_dpi_stages(unsigned char variable_storage, unsigned char count, const unsigned short *dpi)
{
    ...
    // I think this is a flag that distiguishes fixed DPI from DPI stages.
    // It's always 0x01 when disabling DPI stages and 0x02 when enabling DPI
    // them.
    report.arguments[1] = count >= 2 ? 0x02 : 0x01;
